...
class MpvitDepthDecoder(nn.Module):
    ...
    def forward(self, input_features):
        ...
        # 保存了未经激活的深度图特征，用于后续的扩散模型
        outputs[("disp_diffusion", 0)] = self.convs["dispconv0"](x)
        outputs[("disp_diffusion", 1)] = self.convs["dispconv1"](features["X_04"])
        outputs[("disp_diffusion", 2)] = self.convs["dispconv2"](features["X_13"])
        outputs[("disp_diffusion", 3)] = self.convs["dispconv3"](features["X_22"])
